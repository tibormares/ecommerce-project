...
public class CartController {
    ...
    @PostMapping("/users/cart/products/{productId}/quantity/{quantity}")
    ...
    @PutMapping("/users/cart/products/{productId}/quantity/{operation}")
    ...
    @Operation(summary = "Remove a product from a cart", description = "Completely removes a product from user's cart. Requires authentication.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Product removed successfully"),
            @ApiResponse(responseCode = "401", description = "Unauthorized access"),
            @ApiResponse(responseCode = "404", description = "Cart or product not found")
    })
    @DeleteMapping("/users/cart/product/{productId}")
    public ResponseEntity<String> deleteProductFromCart(@Parameter(description = "ID of the product to remove") @PathVariable Long productId) {
        Cart cart = cartRepository.findCartByEmail(authUtil.loggedInEmail());
        String status = cartService.deleteProductFromCart(cart.getCartId(), productId);
